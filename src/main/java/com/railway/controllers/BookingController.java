...
public class BookingController {
	...
	public String paymentSuccessfull(HttpServletRequest request, @PathVariable String trainNo,
			@ModelAttribute("selectedTrain") TrainDTO train) throws CommonTrainException {
		...
		// creating Ticket
		ticketService.createTicket(user, train.convertToEntity(), listOfPassenger,
				(int) session.getAttribute(TOTAL_AMOUNT));
		ticketService.decreaseTicketCounts(train.convertToEntity(), listOfPassenger);

		logger.info("Ticket Booked Successful for user with id {} for the train No {}", user.getEmail(),
				train.getTrainNo());
